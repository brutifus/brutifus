...
# --------------------------------------------------------------------------------------------------
def get_linear_WCS_corr(fn, obstime=None, suffix=None, target=None, verbose=True):
    ''' Identify the linear offset between Gaia and the WCS solution of a given 2D image.

    Args:
        fn (str): 2D FITS image filename (and location)
        obstime (astropy.Time, optional): Observing time and date. Defaults to None.
        suffix (str, optional): the tag of this step (for plot filename). Defaults to None.
        target (str, optional): target name (for plot filename). Defaults to None.
        verbose (bool optional): print some info to screen. Defaults to False.

    Returns:
        tuple: (dx,dy), the linear offsets in pixels
    ...
    _, wl_median, wl_std = sigma_clipped_stats(wl_im, sigma=3.0)
    ...
    core_coord = SkyCoord(ra=header['CRVAL1'] + header['CD1_1'] * (0.5*header['NAXIS1'] -
                                                                   header['CRPIX1']),
                          dec=header['CRVAL2'] + header['CD2_2'] * (0.5*header['NAXIS2'] -
                                                                    header['CRPIX2']),
    ...
    for (im, entries) in [(im_obs, sources_list), (im_ref, gaia_cat_pix)]:
        ...
        d = np.sqrt((x_ind - entry[0])**2 + (y_ind - entry[1])**2)
    ...
    _, _, std = sigma_clipped_stats(im_corr, sigma=10.0)
