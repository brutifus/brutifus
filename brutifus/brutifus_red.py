...
def D_fm90(x,gamma,x0):
    ...
    Args:
        x (float|ndarray): Wavelength in 1/microns.
        gamma (float): The gamma factor.
        x0 (float): The x0 factor.
...
def f99_alebv(lams, rv=3.1):
    ...
    Args:
        lams (ndarray): the wavelengths in Angstroem.
        rv (float, optional): the Rv value. Defaults to 3.1.
...
def fd05_elvebv(lams, rv = 3.08, rva = 4.3): # lams in Angstroem
    ...
    Args:
        lams (ndarray): the wavelengths at which to evaluate the attenutation.
        rv (float, optional): the Rv value, following Fischera & Dopita. Defaults to 3.08
        rva (float, optional): The Rva value, following Fischera & Dopita. Defaults to 4.3.

    Returns:
        ndarray: the value of E(lambda-V)/E(B-V), as in Table 1 and Fig.3 of the article.
...
def cal00_ke(lams, rv):
    ...
    Args:
        lams (ndarray): the wavelength array in Angstroem.
        rv (float): the Rv value

    Returns:
       ndarray: ke from Calzetti (2000)
    ...
    for (i, lam) in enumerate(lams):
        if lam / 1.0e4 < 0.63 and lam / 1.0e4 >= 0.12:
            ke[i] = 2.659 * (-2.156 + 1.509 * my_lams[i] - 0.198 * my_lams[i]**2 +
                             0.011 * my_lams[i]**3) + rv
        elif lam / 1.0e4 < 2.20 and lam / 1.0e4 >= 0.63:
...
def ccm89_alav(lams, rv):
    ...
    for (i, my_lam) in enumerate(my_lams):
        x = my_lam
        if 0.3 <= x < 1.1:
            a[i] = +0.574 * x**1.61
            b[i] = -0.527 * x**1.61
...
def alam(lams, ab, av, curve='f99', rv=None, rva=4.3):
    ...
    Args:
        lams (ndarray): the values at which to evaluate the attenuation/extinction.
        ab (float): the extinction in B (e.g. according to NED)
        av (float): the extinction in V (e.g. according to NED)
        curve (str, optional): extinction/attenuation curve to use: 'f99', 'fd05', 'cal00',
            'cal00*', 'ccm89'. Defaults to 'f99'.
        rv (float, optional): the value of Rv. Defaults to None.
        rva (float, optional): the value of Rva, if using curve='fd05'.Defaults to 4.3
...
def galactic_red(lams, ab, av, curve='f99', rv=None, rva=4.5):
    ...
    Args:
        ...
        curve (str, optional): extinction/attenuation curve to use: 'f99', 'fd05', 'cal00',
            'cal00*', 'ccm89'. Defaults to 'f99'.
        rv (float, optional): Rv value. Defaults to None.
        rva (float, optional): The value of Rva, if using curve='fd05'. Defaults to 4.5

    Returns:
        ndarray: the ratio of Flux_(unreddened)/ Flux_(observed).
...
def extragalactic_red(lam, hahb, hahb_0, curve='fd05', rv=None, rva=4.3):
    ...
    Args:
        ...
        hahb (ndarray): the observed Halpha/Hbeta flux ratio. Can be a 2-D map.
        hahb_0 (float): the reference theoretical unreddened Halpha/Hbeta flux ratio.
        curve (str, optional): which extinction/attenuation curve to use. Defaults to 'fd05'.
        rv (float, optional): Rv value. Defaults to None.
        rva (float, optional): The value of Rva, if using curve='fd05'. Defaults to 4.3

    Returns:
        ndarray: the ratio of Flux_(unattenuated)/ Flux_(attenuated)
    ...
    raise Exception('Ouch ! extragalactic_red() needs to be fixed')

    # TODO: need to fix the ha and hb to the waveelngth of these lines ... this looks like
    # a left over from brutifus that has not been proted yet.
    #if curve == 'fd05':
...
def hahb_to_av(hahb, hahb_0, curve='fd05', rv=None, rva=4.3):
    ...
    Args:
        ha_hb (ndarray): the flux ratio of Halpha to Hbeta (NOT the log!)
        ha_hb_0 (ndarray): the theoretical value of Halpha/Hbeta. (2.86 for SB, more for AGNs)
        curve (str, optional): which reddeing law to use ? Defaults to 'fd05'
        rv (float, optional): the value of Rv to use. None for the default. Defaults to None.
        rva (float, optional): The value of Rva, if using curve='fd05'. Defaults to 4.3

    Returns:
        ndarray: the corrseponding values of Av.
    ...
    raise Exception('Ouch ! hahb_to_av() needs to be fixed !')
    # TODO: need to fix the ha and hb to the waveelngth of these lines ... this looks like
    # a left over from brutifus that has not been proted yet.
    #if curve == 'fd05':
    #    ehavebv = fd05_elvebv(ha)
